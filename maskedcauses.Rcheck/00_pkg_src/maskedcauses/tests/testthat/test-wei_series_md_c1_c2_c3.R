# Tests for wei_series_md_c1_c2_c3 likelihood model

# Helper function to create Weibull test data
create_wei_test_data <- function(n = 100, shapes = c(1.2, 1.5, 0.8),
                                  scales = c(100, 150, 200), seed = 42) {
  set.seed(seed)
  m <- length(shapes)
  stopifnot(length(scales) == m)

  # Generate component lifetimes from Weibull distributions
  comp_lifetimes <- matrix(NA, nrow = n, ncol = m)
  for (j in seq_len(m)) {
    comp_lifetimes[, j] <- rweibull(n, shape = shapes[j], scale = scales[j])
  }

  # System lifetime is minimum of component lifetimes
  t <- apply(comp_lifetimes, 1, min)

  # Failed component is the one with minimum lifetime
  failed_comp <- apply(comp_lifetimes, 1, which.min)

  # Create candidate sets - include the failed component
  # plus some random other components

  cand_matrix <- matrix(FALSE, nrow = n, ncol = m)
  for (i in seq_len(n)) {
    cand_matrix[i, failed_comp[i]] <- TRUE
    # Randomly include other components with probability 0.3
    for (j in seq_len(m)) {
      if (j != failed_comp[i] && runif(1) < 0.3) {
        cand_matrix[i, j] <- TRUE
      }
    }
  }

  # Build data frame
  df <- data.frame(t = t, omega = "exact", stringsAsFactors = FALSE)
  for (j in seq_len(m)) {
    df[[paste0("x", j)]] <- cand_matrix[, j]
  }

  df
}

# Helper to create censored Weibull test data
create_wei_censored_data <- function(n = 100, shapes = c(1.2, 1.5, 0.8),
                                      scales = c(100, 150, 200),
                                      tau = 50, seed = 42) {
  df <- create_wei_test_data(n, shapes, scales, seed)

  # Apply right censoring
  censored <- df$t > tau
  df$t[censored] <- tau
  df$omega[censored] <- "right"

  # For censored observations, empty candidate set
  m <- length(shapes)
  for (j in seq_len(m)) {
    df[[paste0("x", j)]][censored] <- FALSE
  }

  df
}


# ==============================================================================
# Tests for model constructor
# ==============================================================================

test_that("wei_series_md_c1_c2_c3 constructor creates correct object", {
  model <- wei_series_md_c1_c2_c3()

  expect_s3_class(model, "wei_series_md_c1_c2_c3")
  expect_s3_class(model, "series_md")
  expect_s3_class(model, "likelihood_model")
  expect_equal(model$lifetime, "t")
  expect_equal(model$omega, "omega")
  expect_equal(model$lifetime_upper, "t_upper")
  expect_equal(model$candset, "x")
})

test_that("wei_series_md_c1_c2_c3 constructor accepts custom column names", {
  model <- wei_series_md_c1_c2_c3(
    lifetime = "time",
    omega = "obs_type",
    candset = "cand"
  )

  expect_equal(model$lifetime, "time")
  expect_equal(model$omega, "obs_type")
  expect_equal(model$candset, "cand")
})

test_that("wei_series_md_c1_c2_c3 constructor accepts initial parameters", {
  shapes <- c(1.2, 1.5, 0.8)
  scales <- c(100, 150, 200)
  model <- wei_series_md_c1_c2_c3(shapes = shapes, scales = scales)

  expect_equal(model$shapes, shapes)
  expect_equal(model$scales, scales)
})


# ==============================================================================
# Tests for loglik method
# ==============================================================================

test_that("loglik.wei_series_md_c1_c2_c3 returns a function", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  expect_type(ll_fn, "closure")
})

test_that("loglik returns finite value for valid parameters and data", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_test_data(n = 50)

  # Parameters: (shape1, scale1, shape2, scale2, shape3, scale3)
  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
  expect_type(ll, "double")
})

test_that("loglik returns -Inf for negative shape parameters", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_test_data(n = 50)

  expect_equal(ll_fn(df, par = c(-1.2, 100, 1.5, 150, 0.8, 200)), -Inf)
})

test_that("loglik returns -Inf for negative scale parameters", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_test_data(n = 50)

  expect_equal(ll_fn(df, par = c(1.2, -100, 1.5, 150, 0.8, 200)), -Inf)
})

test_that("loglik returns -Inf for zero parameters", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_test_data(n = 50)

  expect_equal(ll_fn(df, par = c(0, 100, 1.5, 150, 0.8, 200)), -Inf)
  expect_equal(ll_fn(df, par = c(1.2, 0, 1.5, 150, 0.8, 200)), -Inf)
})

test_that("loglik errors on empty data frame", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- data.frame(t = numeric(0), x1 = logical(0), x2 = logical(0))

  expect_error(ll_fn(df, par = c(1, 100, 1, 100)), "df is empty")
})

test_that("loglik errors when lifetime column is missing", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- data.frame(omega = "exact", x1 = c(TRUE, FALSE), x2 = c(FALSE, TRUE),
                   stringsAsFactors = FALSE)

  expect_error(ll_fn(df, par = c(1, 100, 1, 100)), "lifetime variable")
})

test_that("loglik errors when parameter length is wrong", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_test_data(n = 50)

  # 3 components need 6 parameters, giving only 4
  expect_error(ll_fn(df, par = c(1.2, 100, 1.5, 150)), "Expected .* parameters but got")
})

test_that("loglik handles censored data correctly", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  df <- create_wei_censored_data(n = 50, tau = 50)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("loglik value increases when parameters approach true values", {
  # Given: data generated from known parameters
  true_shapes <- c(1.2, 1.5, 0.8)
  true_scales <- c(100, 150, 200)
  true_par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  distant_par <- c(2.0, 500, 2.0, 500, 2.0, 500)

  df <- create_wei_test_data(n = 200, shapes = true_shapes,
                              scales = true_scales, seed = 123)

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  # When: we evaluate at true params vs distant params
  ll_true <- ll_fn(df, par = true_par)
  ll_distant <- ll_fn(df, par = distant_par)

  # Then: likelihood at true params should be higher
  expect_gt(ll_true, ll_distant)
})


# ==============================================================================
# Tests for score method
# ==============================================================================

test_that("score.wei_series_md_c1_c2_c3 returns a function", {
  model <- wei_series_md_c1_c2_c3()
  score_fn <- score(model)

  expect_type(score_fn, "closure")
})

test_that("score returns vector of correct length", {
  model <- wei_series_md_c1_c2_c3()
  score_fn <- score(model)
  df <- create_wei_test_data(n = 50)

  # 3 components = 6 parameters
  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  s <- score_fn(df, par = par)

  expect_length(s, 6)
  expect_true(all(is.finite(s)))
})

test_that("score returns NA for negative parameters", {
  model <- wei_series_md_c1_c2_c3()
  score_fn <- score(model)
  df <- create_wei_test_data(n = 50)

  s <- score_fn(df, par = c(-1.2, 100, 1.5, 150, 0.8, 200))

  expect_true(all(is.na(s)))
})

test_that("score is consistent with numerical gradient", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  score_fn <- score(model)

  df <- create_wei_test_data(n = 50, seed = 789)
  par <- c(1.2, 100, 1.5, 150, 0.8, 200)

  # Compute analytical score
  analytical <- score_fn(df, par)

  # Compute numerical gradient
  numerical <- numDeriv::grad(func = function(theta) ll_fn(df, theta), x = par)

  # They should be close
  expect_equal(analytical, numerical, tolerance = 1e-4)
})


# ==============================================================================
# Tests for hess_loglik method
# ==============================================================================

test_that("hess_loglik.wei_series_md_c1_c2_c3 returns a function", {
  model <- wei_series_md_c1_c2_c3()
  hess_fn <- hess_loglik(model)

  expect_type(hess_fn, "closure")
})

test_that("hessian returns matrix of correct dimensions", {
  model <- wei_series_md_c1_c2_c3()
  hess_fn <- hess_loglik(model)
  df <- create_wei_test_data(n = 50)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  H <- hess_fn(df, par = par)

  expect_true(is.matrix(H))
  expect_equal(dim(H), c(6, 6))
  expect_true(all(is.finite(H)))
})

test_that("hessian is approximately symmetric", {
  model <- wei_series_md_c1_c2_c3()
  hess_fn <- hess_loglik(model)
  df <- create_wei_test_data(n = 50)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  H <- hess_fn(df, par = par)

  expect_equal(H, t(H), tolerance = 1e-6)
})

test_that("hessian is consistent with numerical second derivative", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  hess_fn <- hess_loglik(model)

  df <- create_wei_test_data(n = 50, seed = 202)
  par <- c(1.2, 100, 1.5, 150, 0.8, 200)

  # Compute analytical Hessian (via jacobian of score)
  analytical <- hess_fn(df, par)

  # Compute numerical Hessian
  numerical <- numDeriv::hessian(func = function(theta) ll_fn(df, theta), x = par)

  # They should be close
  expect_equal(analytical, numerical, tolerance = 1e-3)
})


# ==============================================================================
# Tests for assumptions method
# ==============================================================================

test_that("assumptions returns character vector", {
  model <- wei_series_md_c1_c2_c3()
  a <- assumptions(model)

  expect_type(a, "character")
  expect_gt(length(a), 0)
})

test_that("assumptions includes expected assumptions", {
  model <- wei_series_md_c1_c2_c3()
  a <- assumptions(model)

  expect_true(any(grepl("Weibull", a, ignore.case = TRUE)))
  expect_true(any(grepl("series", a, ignore.case = TRUE)))
  expect_true(any(grepl("C1", a)))
  expect_true(any(grepl("C2", a)))
  expect_true(any(grepl("C3", a)))
})


# ==============================================================================
# Tests for edge cases
# ==============================================================================

test_that("loglik handles singleton candidate sets", {
  # Create data where each candidate set contains exactly one component
  df <- data.frame(
    t = c(10, 20, 30),
    omega = c("exact", "exact", "exact"),
    x1 = c(TRUE, FALSE, FALSE),
    x2 = c(FALSE, TRUE, FALSE),
    x3 = c(FALSE, FALSE, TRUE),
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("loglik handles all components in candidate set", {
  # Create data where candidate set contains all components
  df <- data.frame(
    t = c(10, 20, 30),
    omega = c("exact", "exact", "exact"),
    x1 = c(TRUE, TRUE, TRUE),
    x2 = c(TRUE, TRUE, TRUE),
    x3 = c(TRUE, TRUE, TRUE),
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("model works with custom column names", {
  # Create data with non-standard column names
  df <- data.frame(
    time = c(10, 20, 30),
    obs_type = c("exact", "exact", "exact"),
    cand1 = c(TRUE, FALSE, FALSE),
    cand2 = c(FALSE, TRUE, TRUE),
    cand3 = c(TRUE, TRUE, TRUE),
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3(
    lifetime = "time",
    omega = "obs_type",
    candset = "cand"
  )

  ll_fn <- loglik(model)
  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("model handles two-component systems", {
  df <- data.frame(
    t = c(10, 20, 30),
    omega = c("exact", "exact", "exact"),
    x1 = c(TRUE, TRUE, FALSE),
    x2 = c(FALSE, TRUE, TRUE),
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  score_fn <- score(model)
  hess_fn <- hess_loglik(model)

  par <- c(1.2, 100, 1.5, 150)

  ll <- ll_fn(df, par)
  s <- score_fn(df, par)
  H <- hess_fn(df, par)

  expect_true(is.finite(ll))
  expect_length(s, 4)
  expect_equal(dim(H), c(4, 4))
})

test_that("model handles single observation", {
  df <- data.frame(
    t = 15.5,
    omega = "exact",
    x1 = TRUE,
    x2 = FALSE,
    x3 = TRUE,
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("model handles shape = 1 (exponential special case)", {
  # When all shapes are 1, Weibull reduces to exponential
  df <- create_wei_test_data(n = 50, shapes = c(1, 1, 1), scales = c(100, 150, 200))

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(1, 100, 1, 150, 1, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("model handles shape < 1 (decreasing hazard)", {
  df <- create_wei_test_data(n = 50, shapes = c(0.5, 0.7, 0.8), scales = c(100, 150, 200))

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(0.5, 100, 0.7, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})

test_that("model handles shape > 1 (increasing hazard)", {
  df <- create_wei_test_data(n = 50, shapes = c(2.0, 1.5, 3.0), scales = c(100, 150, 200))

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  par <- c(2.0, 100, 1.5, 150, 3.0, 200)
  ll <- ll_fn(df, par = par)

  expect_true(is.finite(ll))
})


# ==============================================================================
# Tests for rdata method
# ==============================================================================

test_that("rdata.wei_series_md_c1_c2_c3 returns a function", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  expect_type(rdata_fn, "closure")
})

test_that("rdata generates data frame with correct structure", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  set.seed(42)
  # theta = (shape1, scale1, shape2, scale2, shape3, scale3)
  df <- rdata_fn(theta = c(1.2, 100, 1.5, 150, 0.8, 200), n = 100)

  expect_s3_class(df, "data.frame")
  expect_equal(nrow(df), 100)
  expect_true("t" %in% names(df))
  expect_true("omega" %in% names(df))
  expect_true("x1" %in% names(df))
  expect_true("x2" %in% names(df))
  expect_true("x3" %in% names(df))
  expect_true(all(df$omega %in% c("exact", "right")))
})

test_that("rdata respects custom column names from model", {
  model <- wei_series_md_c1_c2_c3(
    lifetime = "time",
    omega = "obs_type",
    candset = "cand"
  )
  rdata_fn <- rdata(model)

  set.seed(42)
  df <- rdata_fn(theta = c(1.2, 100, 1.5, 150), n = 50)

  expect_true("time" %in% names(df))
  expect_true("obs_type" %in% names(df))
  expect_true("cand1" %in% names(df))
  expect_true("cand2" %in% names(df))
})

test_that("rdata applies right-censoring correctly", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  set.seed(42)
  df <- rdata_fn(theta = c(1.2, 100, 1.5, 150, 0.8, 200), n = 200, tau = 30)

  # Check that censored observations exist and have correct properties
  censored <- df$omega == "right"
  if (any(censored)) {
    # Censored times should equal tau
    expect_true(all(df$t[censored] == 30))
    # Censored observations should have empty candidate sets
    expect_true(all(!df$x1[censored]))
    expect_true(all(!df$x2[censored]))
    expect_true(all(!df$x3[censored]))
  }

  # Exact observations should have at least one component in candidate set
  exact <- df$omega == "exact"
  if (any(exact)) {
    cand_sums <- df$x1[exact] + df$x2[exact] + df$x3[exact]
    expect_true(all(cand_sums >= 1))
  }
})

test_that("rdata generates candidate sets satisfying C1", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  # With p = 0, only the failed component should be in candidate set
  set.seed(42)
  df <- rdata_fn(theta = c(1.2, 100, 1.5, 150, 0.8, 200), n = 100, p = 0)

  # Each exact observation should have exactly one component in candidate set
  exact <- df$omega == "exact"
  cand_sums <- df$x1[exact] + df$x2[exact] + df$x3[exact]
  expect_true(all(cand_sums == 1))
})

test_that("rdata errors on invalid parameters", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  # Negative shape
  expect_error(rdata_fn(theta = c(-1, 100, 1.5, 150), n = 100))
  # Zero scale
  expect_error(rdata_fn(theta = c(1, 0, 1.5, 150), n = 100))
  # Odd number of parameters
  expect_error(rdata_fn(theta = c(1, 100, 1.5), n = 100))
})

test_that("rdata generated data can be used with loglik", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)
  ll_fn <- loglik(model)

  set.seed(42)
  theta <- c(1.2, 100, 1.5, 150, 0.8, 200)
  df <- rdata_fn(theta = theta, n = 100, tau = 300, p = 0.3)
  ll <- ll_fn(df, par = theta)

  expect_true(is.finite(ll))
})


# ==============================================================================
# Tests for fim method
# ==============================================================================

test_that("fim returns a function", {
  model <- wei_series_md_c1_c2_c3()
  fim_fn <- fim(model)

  expect_type(fim_fn, "closure")
})

test_that("fim returns matrix of correct dimensions", {
  model <- wei_series_md_c1_c2_c3()
  fim_fn <- fim(model)

  set.seed(42)
  theta <- c(1.2, 100, 1.5, 150, 0.8, 200)
  I <- fim_fn(theta = theta, n_obs = 50, n_samples = 30)

  expect_true(is.matrix(I))
  expect_equal(dim(I), c(6, 6))
  expect_true(all(is.finite(I)))
})


# ==============================================================================
# Test: Single-component system (m=1)
# ==============================================================================

test_that("Weibull model works with single-component system (m=1)", {
  df <- data.frame(
    t = c(10, 20, 30),
    omega = c("exact", "exact", "exact"),
    x1 = c(TRUE, TRUE, TRUE),
    stringsAsFactors = FALSE
  )

  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)
  score_fn <- score(model)
  hess_fn <- hess_loglik(model)

  par <- c(1.2, 100)

  ll <- ll_fn(df, par = par)
  s <- score_fn(df, par = par)
  H <- hess_fn(df, par = par)

  expect_true(is.finite(ll))
  expect_length(s, 2)
  expect_equal(dim(H), c(2, 2))
})

test_that("Weibull rdata works with single-component system (m=1)", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  set.seed(42)
  df <- rdata_fn(theta = c(1.2, 100), n = 50, tau = 200)

  expect_true("x1" %in% names(df))
  expect_true(all(df$x1[df$omega == "exact"]))
})


# ==============================================================================
# Test: High censoring fraction
# ==============================================================================

test_that("Weibull loglik is finite with high censoring", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  set.seed(42)
  df <- rdata_fn(theta = c(1.2, 100, 1.5, 150, 0.8, 200), n = 200, tau = 5, p = 0.3)
  cens_frac <- mean(df$omega == "right")
  expect_gt(cens_frac, 0.5)

  ll_fn <- loglik(model)
  score_fn <- score(model)

  par <- c(1.2, 100, 1.5, 150, 0.8, 200)
  ll <- ll_fn(df, par = par)
  s <- score_fn(df, par = par)

  expect_true(is.finite(ll))
  expect_true(all(is.finite(s)))
})


# ==============================================================================
# Test: Error message validation
# ==============================================================================

test_that("Weibull error messages are informative", {
  model <- wei_series_md_c1_c2_c3()
  ll_fn <- loglik(model)

  df_empty <- data.frame(t = numeric(0), x1 = logical(0))
  expect_error(ll_fn(df_empty, par = c(1, 100)), "df is empty")

  df_no_t <- data.frame(omega = "exact", x1 = c(TRUE, FALSE),
                         stringsAsFactors = FALSE)
  expect_error(ll_fn(df_no_t, par = c(1, 100)), "lifetime variable")

  df <- data.frame(
    t = c(1, 2), omega = c("exact", "exact"),
    x1 = c(TRUE, FALSE), x2 = c(FALSE, TRUE),
    stringsAsFactors = FALSE
  )
  expect_error(ll_fn(df, par = c(1, 100, 1)), "Expected .* parameters but got")
})


# ==============================================================================
# Coverage gap: score parameter count mismatch
# ==============================================================================

test_that("score errors when parameter count is wrong", {
  model <- wei_series_md_c1_c2_c3()
  score_fn <- score(model)

  df <- data.frame(
    t = c(1, 2), omega = c("exact", "exact"),
    x1 = c(TRUE, FALSE), x2 = c(FALSE, TRUE),
    stringsAsFactors = FALSE
  )

  # 2 components need 4 parameters (shape1, scale1, shape2, scale2), giving 2
  expect_error(score_fn(df, par = c(1.2, 100)), "Expected .* parameters but got")
})


# ==============================================================================
# Coverage gap: wei_series_integrand returns 0 for t <= 0
# ==============================================================================

test_that("wei_series_integrand returns 0 for non-positive time", {
  shapes <- c(1.5, 2.0)
  scales <- c(100, 200)
  cind <- c(TRUE, TRUE)

  result <- wei_series_integrand(c(-1, 0, 5), shapes, scales, cind)

  expect_equal(result[1], 0)
  expect_equal(result[2], 0)
  expect_true(result[3] > 0)
})


# ==============================================================================
# Coverage gap: rdata with invalid masking probability p
# ==============================================================================

test_that("rdata errors when p is out of [0,1]", {
  model <- wei_series_md_c1_c2_c3()
  rdata_fn <- rdata(model)

  expect_error(rdata_fn(theta = c(1.2, 100, 1.5, 150), n = 10, p = -0.1),
               "p must be in")
  expect_error(rdata_fn(theta = c(1.2, 100, 1.5, 150), n = 10, p = 1.5),
               "p must be in")
})


# ==============================================================================
# Coverage gap: custom lifetime_upper column name with interval censoring
# ==============================================================================

test_that("Weibull heterogeneous model respects custom lifetime_upper with intervals", {
  model <- wei_series_md_c1_c2_c3(
    lifetime = "time", lifetime_upper = "time_hi",
    omega = "obs_type", candset = "c"
  )
  gen <- rdata(model)

  set.seed(42)
  df <- gen(
    theta = c(1.5, 100, 2.0, 150), n = 200, p = 0.3,
    observe = observe_periodic(delta = 10, tau = 80)
  )

  expect_true("time" %in% names(df))
  expect_true("obs_type" %in% names(df))
  if (any(df$obs_type == "interval")) {
    expect_true("time_hi" %in% names(df))
  }

  ll_fn <- loglik(model)
  ll_val <- ll_fn(df, par = c(1.5, 100, 2.0, 150))
  expect_true(is.finite(ll_val))
})
